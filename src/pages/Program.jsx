...
export default function Program() {
  ...
  const loadPrograms = async (page = 1) => {
    try {
      setLoading(true);
      const res = await programmeService.getAll({ page, search, type: selectedType, per_page: perPage });
      if (res && res.meta) {
                ...
                <tbody>
                  {filtered.length === 0 ? (
                    ...
                    filtered.map((program, i) => (
                      ...
                      >
                        <td className="px-6 py-4 font-mono text-xs text-gray-400">#{program.id}</td>
                        <td className="px-4 py-4 font-medium text-gray-800">{program.titre}</td>
