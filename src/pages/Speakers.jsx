...
export default function Speakers() {
  ...
  const loadMembers = async (page = 1) => {
    try {
      setLoading(true);
      const res = await speakerService.getAll({ page, search, type: selectedType, per_page: perPage });
      if (res && res.meta) {
        setMembers(res.data);
            ...
            <tbody>
              {filtered.length === 0 ? (
                ...
                filtered.map((member, i) => (
                  ...
                  >
                    <td className="px-6 py-4 font-mono text-xs text-gray-400">#{member.id}</td>
                    <td className="px-4 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[#1a7a3c] to-[#2d5a3f] flex items-center justify-center text-white font-semibold overflow-hidden border border-gray-100 bg-white">
                          {member.photo_url ? (
